...
- **Coturn (Global Relay)** - Guaranteed connection (fallback for restrictive networks)
...
- Cloudflare D1 (Analytics & User Stats)
- Coturn (TURN Server for Relay)
...
**User Experience:**
- Tap button → Command sent instantly
...
│   │   ├── MessageBubble.jsx
│   │   ├── CommandInput.jsx
│   │   ├── TemplateCommands.jsx    # NEW: Quick action buttons
│   │   └── DiffViewer.jsx          # NEW: Side-by-side diff modal
│   └── Settings/
│       └── SettingsPanel.jsx
├── lib/
...
│   ├── queue.js         # NEW: Offline queue management
│   └── wakeLock.js      # NEW: Screen keep-alive
├── store/
...
## 10. Advanced Intelligence & UX Strategy

### 10.1 Prompt Engineering: The "Genius" Core
*Objective: Teach the LLM not just what to do, but the context and rules.*

**1. Context Flow & Tiered Context Stack:**
Instead of just sending code, we send a **Tiered Context Stack**:
- **Layer 1:** Current file content.
- **Layer 2:** Last 5 user actions (e.g., "Opened Settings.jsx", "Edited auth.js").
- **Layer 3:** Last 3 error messages.
*Result:* The AI acts as a debugger, understanding *why* you are making a change, not just *what* the change is.

**2. Security & Trust (".mcprules"):**
- **Rule Injection:** The project root can contain a `.mcprules` file.
- **Usage:** Users define strict rules (e.g., "Never use 'any' type", "Must use TailwindCSS").
- **Enforcement:** These rules are injected into every prompt to ensure compliance.

**3. "Wow!" Effect - Diff-First Output:**
- **Optimization:** The LLM is trained to return *only* the changed lines in a git-diff format, not the full file.
- **Benefit:** Drastically reduces data transfer (faster on mobile/Workers) and is ready for the "Diff Viewer" immediately.

**4. Ease of Use - Auto-Classification:**
- **Smart Titles:** The LLM automatically generates a short, descriptive title for every change (e.g., "Fix: Login function type issue").
- **Mobile UX:** This title appears on the mobile confirmation screen, so the user knows exactly what they are approving at a glance.

### 10.2 Advanced UX: Fluid Interactions
*Philosophy: Perfect UX is removing unnecessary steps.*

**A. Instant & Secure Setup (QR Match):**
- **Problem:** Typing server addresses and keys is clunky.
- **Solution:** The login screen features a **Camera Button**.
- **Flow:** User scans a QR code on their desktop. Connection is instant.
- **Slogan:** "Your coding environment, one QR scan away."
- **Result:** The mobile device instantly transforms into a "Project Control Panel".

**B. Mobile Code Control Center:**
- **Action Pending Mode:** Upon sending a command, the interface shifts from "Chat" to "Processing".
- **Process Flow View:** Real-time micro-updates appear at the top:
  - `[Status] Collecting context...`
  - `[Status] Waiting for LLM...`
  - `[Status] Calculating Diff...`
- **Voice Command 2.0:** A dedicated microphone button above the keyboard converts speech to text, cleans it up into a structured prompt, and shows a preview before sending.

**C. Data-Driven Insights (D1 Analysis):**
- **Usage Tracking:** Analyze command frequency and error rates using Cloudflare D1.
- **User Value:** Display stats on the Mobile Dashboard: *"This month, you saved 12 hours of coding time with AI."* (Justifies the value).

**D. Infrastructure as Marketing (Coturn):**
- **Reliability:** Unlike other tools that fail on restrictive office networks, MobileCoderMCP uses its own **Global Relay Infrastructure (Coturn)**.
- **Promise:** "Guaranteed connection, anywhere."
...
- 🌍 Guaranteed Connection (Global Relay)
...
- ✅ Simple setup (5 min)
- ✅ Real P2P connection
- ✅ Template Commands (one-tap actions)
- ✅ Diff Viewer + Preview (code review on mobile)
- ✅ Mobile Keep Alive (no screen lock)
- ✅ Offline Queue (work without internet)
